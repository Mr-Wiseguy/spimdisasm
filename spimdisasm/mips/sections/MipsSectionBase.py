...
from ...common import FileSectionType, SymbolSpecialType, ContextSymbol, GlobalConfig, Utils

from ..MipsFileBase import FileBase

class SectionBase(FileBase):
    sectionSizes: dict[FileSectionType, int]
    words: list[int]
    def checkWordIsASymbolReference(self, word: int) -> bool:
        ...
        if contextSym is not None:
            ...
            if symType in {SymbolSpecialType.function, SymbolSpecialType.branchlabel, SymbolSpecialType.jumptablelabel}:
    ...
    def processStaticRelocs(self) -> None:
        for i in range(self.sizew):
            ...
            if self.sectionType == FileSectionType.Rodata and sectionType == FileSectionType.Text:
    ...
    def _stringGuesser(self, contextSym: ContextSymbol, localOffset: int) -> bool:
        ...
        _, rawStringSize = Utils.decodeBytesToStrings(self.bytes, localOffset, self.stringEncoding)
        if rawStringSize < 0:
    ...
    def _pascalStringGuesser(self, contextSym: ContextSymbol, localOffset: int) -> bool:
        ...
        currentVram = self.getVramOffset(localOffset)
        currentVrom = self.getVromOffset(localOffset)
        _, rawStringSize = Utils.decodeBytesToPascalStrings(self.bytes, localOffset, self.stringEncoding, terminator=0x20)
        if rawStringSize < 0:
            # String can't be decoded
            return False
